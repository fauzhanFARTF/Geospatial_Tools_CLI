        ...
        try:
            ...
            for layer in layers:
                try:
                    ...
                    zip_filename = os.path.join(gdb_output_folder, f"{layer}.zip")
                    with zipfile.ZipFile(zip_filename, 'w', zipfile.ZIP_DEFLATED) as zipf:
                        for shp_file in glob.glob(os.path.join(layer_folder, "*.*")):
                            zipf.write(shp_file, os.path.relpath(shp_file, gdb_output_folder))

                    print(f"✅ Layer {layer} berhasil dikonversi ke {zip_filename}")
